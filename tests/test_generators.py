import pytest

from src.generators import (card_number_generator, filter_by_currency,
                            transaction_description)

# Модуль generators

# Тесты функции filter_by_currency
def test_filter_by_currency_USD(transactions):
    result = filter_by_currency(transactions, 'USD')
    assert next(result) == {'id': 939719570, 'state': 'EXECUTED', 'date': '2018-06-30T02:08:58.425572',
                            'operationAmount': {'amount': '9824.07', 'currency': {'name': 'USD', 'code': 'USD'}},
                            'description': 'Перевод организации', 'from': 'Счет 75106830613657916952',
                            'to': 'Счет 11776614605963066702'}
    assert next(result) == {"id": 142264268, "state": "EXECUTED", "date": "2019-04-04T23:20:05.206878",
                            "operationAmount": {"amount": "79114.93", "currency": {"name": "USD", "code": "USD"}},
                            "description": "Перевод со счета на счет", "from": "Счет 19708645243227258542",
                            "to": "Счет 75651667383060284188"}
    assert next(result) == {"id": 895315941, "state": "EXECUTED", "date": "2018-08-19T04:27:37.904916",
                            "operationAmount": {"amount": "56883.54", "currency": {"name": "USD", "code": "USD"}},
                            "description": "Перевод с карты на карту", "from": "Visa Classic 6831982476737658",
                            "to": "Visa Platinum 8990922113665229"}


def test_filter_by_currency_RUB(transactions):
    result = filter_by_currency(transactions, 'RUB')
    assert next(result) == {"id": 873106923, "state": "EXECUTED", "date": "2019-03-23T01:09:46.296404",
                            "operationAmount": {"amount": "43318.34", "currency": {"name": "руб.", "code": "RUB"}},
                            "description": "Перевод со счета на счет", "from": "Счет 44812258784861134719",
                            "to": "Счет 74489636417521191160"}
    assert next(result) == {"id": 594226727, "state": "CANCELED", "date": "2018-09-12T21:27:25.241689",
                            "operationAmount": {"amount": "67314.70", "currency": {"name": "руб.", "code": "RUB"}},
                            "description": "Перевод организации", "from": "Visa Platinum 1246377376343588",
                            "to": "Счет 14211924144426031657"}


def test_filter_by_currency_not_defined(transactions):
    result = filter_by_currency(transactions)
    assert next(result) == "Вы не ввели валюту для сортировки транзакций"


def test_filter_by_currency_is_missing(transactions):
    result = filter_by_currency(transactions, 'FRT')
    assert next(result) == "Данной валюты нет в списке транзакций"


# Тесты функции transaction_description
def test_transaction_description_try(transactions):
    result = transaction_description(transactions)
    assert next(result) == "Перевод организации"
    assert next(result) == "Перевод со счета на счет"
    assert next(result) == "Перевод со счета на счет"
    assert next(result) == "Перевод с карты на карту"
    assert next(result) == "Перевод организации"


# Тесты функции card_number_generator
@pytest.mark.parametrize('value1, value2, expented', [(1, 10, ['0000 0000 0000 0001', '0000 0000 0000 0002', '0000 0000 0000 0003',
                                                               '0000 0000 0000 0004', '0000 0000 0000 0005', '0000 0000 0000 0006',
                                                               '0000 0000 0000 0007', '0000 0000 0000 0008', '0000 0000 0000 0009',
                                                               '0000 0000 0000 0010']),
                                                      (9999999999999998, 9999999999999999, ['9999 9999 9999 9998', '9999 9999 9999 9999'])
                                                      ]
                         )
def test_card_number_generator_try(value1, value2, expented):
    assert card_number_generator(value1, value2) == expented


def test_card_number_generator_negative_value():
    assert card_number_generator(-11, 12) == "Введено некорректное значение для генерации номера карты"


def test_card_number_generator_exceeding_limits():
    assert card_number_generator(11, 9999999999999999999999) == "Введено некорректное значение для генерации номера карты"


def test_card_number_generator_mixed_up():
    assert card_number_generator(100, 12) == "Введено некорректное значение для генерации номера карты"
